...
impl SleighContext {
    ...
    pub fn load_image<T: Into<Image> + Clone>(&mut self, img: T) -> Result<SleighImage, JingleSleighError>{
        self.ctx.pin_mut().loadImage(img.clone().into()).map_err(|_| ImageLoadError).map(SleighImage::new)
    }
