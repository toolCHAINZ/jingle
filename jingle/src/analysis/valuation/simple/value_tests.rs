...
fn load_preserves_size() {
    ...
    let node = Value::Load(Load(Rc::new(child), 4, 1));
...
fn left_shift_const_folding() {
    ...
    let result = Value::IntLeftShift(IntLeftShiftExpr(
        Rc::new(Value::const_(8, 8)),
        Rc::new(Value::const_(2, 8)),
        8,
    ))
    .simplify();
...
fn left_shift_overflow() {
    ...
    let result = Value::IntLeftShift(IntLeftShiftExpr(
        Rc::new(Value::const_(0xFF, 8)),
        Rc::new(Value::const_(64, 8)), // 8 bytes * 8 = 64 bits
        8,
    ))
    .simplify();
    assert_eq!(result, Value::make_const(0, 8));
...
fn left_shift_with_masking() {
    ...
    let result = Value::IntLeftShift(IntLeftShiftExpr(
        Rc::new(Value::make_const(0xFF, 1)), // 1 byte
        Rc::new(Value::const_(4, 8)),
        1,
    ))
    .simplify();
...
fn right_shift_fills_with_zeros() {
    ...
    // 0xF0 >> 4 = 0x0F
    let result = Value::IntRightShift(IntRightShiftExpr(
        Rc::new(Value::make_const(0xF0, 1)),
        Rc::new(Value::const_(4, 8)),
        1,
    ))
    .simplify();
...
fn signed_right_shift_const_folding_negative() {
    ...
    let result = Value::IntSignedRightShift(IntSignedRightShiftExpr(
        Rc::new(Value::make_const(0xF0u64 as i64, 1)),
        Rc::new(Value::const_(2, 8)),
        1,
    ))
    .simplify();
...
fn signed_right_shift_overflow_positive() {
    ...
    let result = Value::IntSignedRightShift(IntSignedRightShiftExpr(
        Rc::new(Value::const_(42, 8)),
        Rc::new(Value::const_(64, 8)),
        8,
    ))
    .simplify();
    assert_eq!(result, Value::make_const(0, 8));
...
fn signed_right_shift_overflow_negative() {
    ...
    let result = Value::IntSignedRightShift(IntSignedRightShiftExpr(
        Rc::new(Value::make_const(0x80u64 as i64, 1)),
        Rc::new(Value::const_(8, 8)),
        1,
    ))
    .simplify();
...
fn signed_right_shift_preserves_sign_bit() {
    ...
    let result = Value::IntSignedRightShift(IntSignedRightShiftExpr(
        Rc::new(Value::make_const(0xFFu64 as i64, 1)),
        Rc::new(Value::const_(4, 8)),
        1,
    ))
    .simplify();
...
fn signed_right_shift_small_negative() {
    ...
    let result = Value::IntSignedRightShift(IntSignedRightShiftExpr(
        Rc::new(Value::make_const(0xFEu64 as i64, 1)),
        Rc::new(Value::const_(1, 8)),
        1,
    ))
    .simplify();
    assert_eq!(result, Value::make_const(0xFFu64 as i64, 1));
